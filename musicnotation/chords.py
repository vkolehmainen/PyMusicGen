...
Useful Functions
 * determine - Can recognize all the chor that can be generated with \
from_shorthand (a lot) and their inversions.
 * from_shorthand - Generates chords from shorthand (eg. 'Cmin7')
 
-------------------------------------------
 Add is_valid
 Edit from_shorthand to recognize inverted chords 
-------------------------------------------
"""

import musicnotation.intervals as intervals
import musicnotation.notes as notes
import musicnotation.keys as keys
from musicnotation.mt_exceptions import NoteFormatError, FormatError
...
def from_shorthand(shorthand_string, slash=None, inversion = False):
    ...
    if slash_index != -1:
        body = shorthand_string[:len(name) + slash_index]
        bass_note = shorthand_string[len(name) + slash_index + 1:]
        shorthand_start = len(name)
        short_chord = shorthand_string[shorthand_start:shorthand_start + slash_index]
        non_inversed_chord = chord_shorthand[short_chord](name)
        if bass_note not in non_inversed_chord:
            raise FormatError('{0} not in chord {1}'\
                              .format(bass_note, body))
        inversion = non_inversed_chord.index(bass_note)
        return from_shorthand(body, None, inversion)
...
def is_valid(shorthand_chord):
    """Recognize valid chord"""
    try:
        from_shorthand(shorthand_chord)
        return True
    except (NoteFormatError, FormatError):
        return False
